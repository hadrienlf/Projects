...
#plot(FranceFormes, main="Carte de la France, d?partements")
...
plot(pop$Dep,pop$x/pop$`Population 2019`,type='h',xlab='Departements',ylab='Nombre d\'accidents liss?')
...
data("pg17testyear1")
test1=pg17testyear1
test1$pol_pay_freq<-as.numeric(test1$pol_pay_freq)
test1$drv_sex1<-as.numeric(test1$drv_sex1)
test1$drv_drv2<-as.numeric(test1$drv_drv2)
test1$pol_coverage<-as.numeric(test1$pol_coverage)
test1$pol_payd<-as.numeric(test1$pol_payd)
test1$pol_usage<-as.numeric(test1$pol_usage)
test1$vh_type<-as.numeric(test1$vh_type)
a=test1$drv_age1
test1$drv_age1=cut(a,breaks=c(18,25,40,60,80,103),include.lowest = TRUE)
b=test1$drv_age_lic1
#table(b)
#on va utiliser les quantiles pour avoir une repartition interressante; en essayant par pas de 0.1 et par pas de 0.2 les 2 semblent interressants:
test1$drv_age_lic1=cut(b,breaks=quantile(b,seq(0,1,0.2)),include.lowest = TRUE)
c=test1$pol_bonus
#table(c)
#on voit les nombres de valeurs pour les differents bonus, on va donc selectionner nous meme les valeurs clefs avec table(test1$pol_bonus):
test1$pol_bonus=cut(c,breaks=c(0.5,0,53,0.60,1,1.5,2.16),include.lowest = TRUE)
#Pour pol_duration aussi on peut reduire le nombre de facteurs:
#table(test1$pol_duration)
#on va utiliser les quantiles:
d=test1$pol_duration
test1$pol_duration=cut(d,breaks=quantile(d,seq(0,1,0.1)),include.lowest = TRUE)
#table(test1$pol_sit_duration)
#on voit avec cette fonction qu'on peut regrouper:
e=test1$pol_sit_duration
test1$pol_sit_duration=cut(e,breaks=quantile(d,seq(0,1,0.1)),include.lowest = TRUE)
...
test1$vh_fuel = as.numeric(test1$vh_fuel)
test1$pol_bonus = as.numeric(test1$pol_bonus)
test1$pol_duration = as.numeric(test1$pol_duration)
test1$pol_sit_duration = as.numeric(test1$pol_sit_duration)
test1$drv_age1 = as.numeric(test1$drv_age1)
test1$drv_age_lic1 = as.numeric(test1$drv_age_lic1)

#test1=test1[-which(is.na(test1$vh_age)),]


test1$Dep=str_sub(test1$pol_insee_code,start=1,end=2)
test1=na.omit(test1)
...
clusterfr$Dep=row.names(clusterfr)
clusterseverite$Dep=row.names(clusterseverite)

test1=merge(test1,clusterfr,by='Dep')
test1=merge(test1,clusterseverite,by='Dep')
#Maintenant on a les variables qui nous donne le groupe de departement dans lequel se situe l'assure. On les renomme pour plus de clarete:
names(test1)[33]='ClusterFreq'
names(test1)[34]='ClusterSev'


#modelefreqONISR = glm.nb(freq~vh_sale_end + vh_value + pol_coverage + vh_fuel + drv_drv2 + 
#                           pol_sit_duration + pol_duration + vh_weight + drv_age_lic1 + 
#                           drv_age1 + pol_bonus + ClusterFreq,
#                         data=Tableaudep)
coef.freqO=coefficients(modelefreqONISR)
coef.sevO=coefficients(modelesevONISR)

# on cree les matrices design  
mfreq0=model.matrix(~ vh_sale_end + vh_value + pol_coverage + vh_fuel + drv_drv2 + 
                      pol_sit_duration + pol_duration + vh_age + vh_weight + drv_age_lic1 + 
                      drv_age1 + pol_bonus + ClusterFreq,test1)

msev0=model.matrix(~pol_coverage+ vh_sale_end + vh_type + vh_value + ClusterSev ,test1)

esp.freqO=exp(mfreq0 %*% coef.freqO)
#calcul de l'esperence de la severite
esp.sevO=exp(msev0 %*% coef.sevO)
#la prime pure
prime=esp.freqO*esp.sevO
primepureONISR=data.frame(test1$id_client,prime)
names(primepureONISR)=c('id_client','prime_pure')
hist(primepureONISR$prime_pure,main='Prime pure',xlab='montant de la prime ONISR')
summary(primepureONISR$prime_pure)
...
# on cree les matrices design  
mfreq=model.matrix(~ vh_sale_end + vh_value + pol_coverage + vh_fuel + drv_drv2 + 
                     pol_sit_duration + pol_duration + vh_age + vh_weight + drv_age_lic1 + 
                     drv_age1 + pol_bonus,test1)

msev=model.matrix(~pol_coverage + vh_sale_end  + 
                    vh_type + vh_value ,test1)
...
#la prime pure
prime=esp.freq*esp.sev
primepure=data.frame(test1$id_client,prime)
names(primepure)=c('id_client','prime_pure')
hist(primepure$prime_pure,main='Prime pure',xlab='montant de la prime')
summary(primepure$prime_pure)
